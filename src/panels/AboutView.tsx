    ...
    <div className="about">
      ...
      <ol className="tries">
        <li>
          ...
          右の制御盤で「視線適応」を OFF、遅延を 300 ms にし、左ドラッグで手を壁（半透明の板）の奥まで押し込みます。
          方式「なし」ではアームが壁で跳ね返りを繰り返し、下のエネルギーのグラフで「通信路の生成エネルギー」（赤）が正に増え続けます。
          「TDPA」に切り替えると跳ね返りが収まり、赤の線は 0 以下にとどまります。
        </li>
        <li>
          <b>見ている場所で重さが変わる。</b>
          ボタンを押さずにマウスを動かすと、その先が「視線」（桃色の点）になり、床に注意の熱分布が広がります。
          緑の箱を見てから手を箱へ運ぶと軽く、よそを見ながら動かすと b(t) が上がって重くなります（ゴーストの遅れで分かります）。
