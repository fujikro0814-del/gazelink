...
1. **遅延で発振させ、TDPA で止める。**
   - 右の制御盤で「視線適応ダンピング」の「有効にする」を外し、遅延を 300 ms にします。左ドラッグで、手を壁（半透明の板）の奥まで押し込みます。
...
2. **見ている場所で重さが変わる。**
   - マウスのボタンを押さずに 3D 画面の上で動かすと、その先が「視線」（桃色の点）になり、床に注意の熱分布が広がります。
   - 緑の箱を見てから手を箱へ運ぶと軽くなります。よそを見ながら動かすと b(t) が上がり、手元が重くなります（水色のゴーストが遅れてついてきます）。
